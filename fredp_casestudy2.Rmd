...
library(tidyverse)
library(dplyr)          ## already included in tidyverse
...
#unknown columns.. not enough background information
#StandardHours EmployeeNumber EmployeeCount Over18
#will remove these columns from dataset
...
data %>% ggplot() +
  geom_point(aes(Gender, MonthlyIncome, color = Attrition)) +
  geom_smooth(aes(Gender, MonthlyIncome, linetype = Attrition)) +
  facet_wrap(~Attrition) +
  ggtitle("Gender vs MonthlyIncome by Attrition")
...
income_exp <- predict(lmodel,income)
exp_i = cbind.data.frame(income$ID,income_exp)
colnames(exp_i) <-c("ID","MonthlyIncome")
write.csv(expo_i,"C:/Users/poonf/OneDrive - Southern Methodist University/6306 doing data science/project 2/Case2PredictionsPOON_Salarytion.csv")
